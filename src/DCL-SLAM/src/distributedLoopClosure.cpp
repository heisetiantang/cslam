...
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * 
	class distributedMapping: loop closure  performRSIntraLoopClosure函数执行机器人内部的循环闭合操作，包括查找循环闭合、计算变换等步骤
* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
...
// detectLoopClosureDistance函数的作用是查找最接近当前帧的历史关键帧，并返回其索引
...
// performIntraLoopClosure函数执行机器人内部的循环闭合操作，包括查找循环闭合、计算变换等步骤
// 如果找到了循环闭合，函数打印一条日志信息，表示找到了机器人内部循环闭合，并输出loop_key0和loop_key1的值。
// 调用calculateTransformation函数来计算从loop_key0到loop_key1之间的变换
...
// loopFindNearKeyframes函数用于从关键帧中找到与给定关键帧附近的关键帧，并将结果存储在near_keyframes点云中
...
// 这段代码定义了一个名为loopFindGlobalNearKeyframes的函数，用于查找全局附近的关键帧
...
// 这段代码定义了一个名为updatePoseEstimateFromNeighbor的函数，用于更新邻居机器人的位姿估计。
...
// 该函数的整体逻辑是在需要进行循环闭合的情况下，以指定的频率循环执行不同类型的循环闭合操作。
// 这样可以实现循环闭合的持续处理和验证，以提高地图构建和定位的准确性和鲁棒性。
void distributedMapping::loopClosureThread()
{
	// Terminate the thread if loop closure are not needed
	// LOG(INFO) << "intra_robot_loop_closure_enable_ " << intra_robot_loop_closure_enable_ <<std::endl;
	// LOG(INFO) << "inter_robot_loop_closure_enable_ " << inter_robot_loop_closure_enable_ <<std::endl;
